...
;;; THIS IS HERE AS A TEMPORARY FIX FOR ISSUE #177:
;;;
;;; https://github.com/Day8/re-com/issues/177
;;;
;;; The only change required is to make split-perc a user param
;;;
...
(def hv-split-args-desc
  [{:name :panel-1         :required true                  :type "hiccup"          :validate-fn string-or-hiccup? :description "markup to go in the left (or top) panel"}
   ...
   {:name :split-perc      :required false                 :type "anything | atom"                                :description "Holds splitter percent (a double value) "}
...
(defn h-split
  ...
  (let [container-id         (gensym "h-split-")
        split-perc           (if split-perc                              ;; splitter position as a percentage of height
                               split-perc
                               (reagent/atom (js/parseInt initial-split)))
